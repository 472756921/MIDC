...
// cdm
const zzzf = ({systemMannger, dispatch})=>{
  ...
  const del = ()=>{
    if(systemMannger.itemData === ''){
      ...
      Confirm('确认删除','删除后将无法恢复，若删除的是治则治法类型，该类型下的类目也将被删除', delItem);
    }
  }
  const add = (e)=>{
    const t = e.target['title'];
    if(t === 'classes') {
      dispatch({type:'systemMannger/changeItemData', itemData:{name:'默认治则治法类型', fClass:0, sysType: 'zzzf', isMenu: 1, type: 1 }});
    } else if(t === 'jb') {
      dispatch({type:'systemMannger/changeItemData', itemData:{name:'新的治则治法',sysType: 'zzzf', isMenu: 0, type: 2 }});
    }
  }
  ...
  return (
    <div className={styles.navBtns}>
      ...
      <Button onClick={add} title='classes'>添加治则治法类型</Button>
      <Button onClick={add} title='jb'>添加治则治法</Button>
      <Button onClick={del}>删除当前治则治法/类型</Button>
      <Divider />
      <Row gutter={16}>
...
export default connect(({systemMannger}) => ({systemMannger}))(zzzf);
