...
const zh = ({systemMannger, dispatch})=>{
  ...
  const del = ()=>{
    if(systemMannger.itemData === ''){
      ...
      Confirm('确认删除','删除后将无法恢复，若删除的是症状类型，该类型下的类目也将被删除', delItem);
    }
  }
  const add = (e)=>{
  ...
  return (
    <div>
      ...
      <Button onClick={add} title='classes'>添加症状类型</Button>
      <Button onClick={add} title='jb'>添加症状</Button>
      <Button onClick={del}>删除当前症状/类型</Button>
      <Divider />
      <Row gutter={16}>
