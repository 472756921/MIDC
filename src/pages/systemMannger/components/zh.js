...
const zh = ({systemMannger, dispatch})=>{
  ...
  const del = ()=>{
    if(systemMannger.itemData === ''){
      ...
      Confirm('确认删除','删除后将无法恢复，若删除的是证候类型，该类型下的类目也将被删除', delItem);
    }
  }
  const add = (e)=>{
    const t = e.target['title'];
    if(t === 'classes') {
      dispatch({type:'systemMannger/changeItemData', itemData:{name:'默认证候类型', fClass:0, sysType: 'zh', isMenu: 1, type: 1 }});
    } else if(t === 'jb') {
      dispatch({type:'systemMannger/changeItemData', itemData:{name:'新的证候',sysType: 'zh', isMenu: 0, type: 2 }});
    }
  }
  ...
  return (
    <div>
      ...
      <Button onClick={add} title='classes'>添加证候类型</Button>
      <Button onClick={add} title='jb'>添加证候</Button>
      <Button onClick={del}>删除当前证候/类型</Button>
      <Divider />
      <Row gutter={16}>
        ...
        <Col xl={12} xxl={8} style={{marginBottom: '10px'}}>
          ...
          <Select value={systemMannger.itemData.fClass} style={{ width: '100%' }}  title='fClass' onChange={changeValue} disabled={systemMannger.itemData.fClass===0?true:false}>
            <Option key={99991} value={0}  disabled={systemMannger.itemData.fClass!==0?true:false}>中医证候类型</Option>
            {
              classes.map((it, i) => {
                return (<Option key={i} value={it.id}>{it.name}</Option>)
              })
